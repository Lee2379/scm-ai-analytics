...
def build_deck() -> None:
    ...
    sp.font.size = Pt(17)
    sp.font.color.rgb = MUTED
    add_card(slide, 9.45, 1.15, 3.1, 3.6, "Project Scope", "Global fashion retail SCM operations with SKU-store inventory risk monitoring.", SAGE)
    add_footer(slide, 1)
    ...
    slide.background.fill.solid()
    slide.background.fill.fore_color.rgb = PAPER
    add_title(slide, "Data Strategy", "Documented SCM simulation supports reproducible operational analysis.")
    ...
    slide.background.fill.solid()
    slide.background.fill.fore_color.rgb = PAPER
    add_title(slide, "Demo Results", "Simulation-based results generated by the local analysis system.")
    ...
    slide.background.fill.solid()
    slide.background.fill.fore_color.rgb = PAPER
    add_title(slide, "Next Steps", "Potential extensions for stronger operational coverage.")
